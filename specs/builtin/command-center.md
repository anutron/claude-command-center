...
| `j` | detail:viewing | Navigate to next todo in the current filtered list (resets viewport scroll to top) |
| `k` | detail:viewing | Navigate to previous todo in the current filtered list (resets viewport scroll to top) |
...
- `j`/`k` always navigate between todos in the current filtered list in detail view; `]`/`[` navigate sources independently
...
- Detail view `j`/`k` navigates within the current filtered list (not all active todos) and resets the viewport scroll position to the top
- Detail view `j`/`k` respects the current triage filter — e.g., on the Focus tab, j/k only cycles through focused items
