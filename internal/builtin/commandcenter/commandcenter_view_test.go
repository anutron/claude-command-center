...
// BUG-146: After starring a todo and dismissing the schedule modal (Escape),
// the tab filter stops working — all items show regardless of selected tab.
//
// Root cause: handleScheduleModal returned NoopAction() for unhandled keys
// (including Tab/Shift+Tab), which let the host process them and switch
// host-level tabs. The fix changes all returns in handleScheduleModal to
// ConsumedAction() so the modal truly intercepts ALL keys.
...
func TestView_BUG146_FilterWorksAfterModalDismiss(t *testing.T) {
	...
	p.HandleKey(specialKeyMsg(tea.KeyTab))
	if p.triageFilter != "inbox" {
		t.Fatalf("expected 'inbox', got %q", p.triageFilter)
	}
