...
func TestStarKey(t *testing.T) {
	...
	// Switch to expanded view so all todos are visible
	p.ccExpanded = true
	p.triageFilter = "all"

	// Ensure all todos start unstarred
...
func TestUnstarConfirmModeWithFutureBookings(t *testing.T) {
	...
	_, err := p.database.Exec(`INSERT INTO cc_todo_bookings (todo_id, event_id, start_time, end_time, duration_min, created_at) VALUES (?, ?, ?, ?, ?, ?)`,
		todo.ID,
		"test-event-123",
