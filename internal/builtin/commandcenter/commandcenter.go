...
func (p *Plugin) viewCommandTab(width, height int) string {
	...
	if p.taskRunnerView && p.detailView && p.cc != nil {
		...
		return renderTaskRunner(&p.styles, *todo, p.taskRunnerMode, p.taskRunnerPerm, p.taskRunnerBudget, p.taskRunnerAutoStart, p.taskRunnerSelectedRow, p.taskRunnerPrompt, viewWidth, viewHeight, taskRunnerProjectDir, p.taskRunnerPickingPath, p.taskRunnerFilteredPaths(), p.taskRunnerPathCursor, p.taskRunnerPathFilter)
		}
	}
